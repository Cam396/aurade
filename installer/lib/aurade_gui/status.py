...
def battery() -> dict[str, object]:
    ...
    for name in supplies:
        ...
        answer["state"] = state
        answer["minutes"] = _battery_minutes(base, state)
    return answer


#: Below this the reading is noise. A battery sitting at full reports a
#: current of a milliamp or two, which divides into a charge of three and a
#: half amp hours and produces a confident "two hundred and twelve hours
#: left". Nothing is the right answer there.
CURRENT_FLOOR = 0.02


def _battery_minutes(base: str, state: str) -> int | None:
    """How long until it is full, or until it is empty.

    Kernels expose either energy/power or charge/current. Read both pairs and
    use the first complete, useful reading.

    Nothing at all when it is neither charging nor discharging, when the
    numbers are missing, or when the rate is too small to divide by. A time
    that is wrong by a factor of fifty is worse than no time.
    """
    if state not in ("Charging", "Discharging"):
        return None
    for now, full, rate in (("energy_now", "energy_full", "power_now"),
                            ("charge_now", "charge_full", "current_now")):
        try:
            have = float(_read(os.path.join(base, now)))
            top = float(_read(os.path.join(base, full)))
            speed = float(_read(os.path.join(base, rate)))
        except ValueError:
            continue
        if top <= 0 or speed <= 0 or speed / top < CURRENT_FLOOR:
            continue
        left = (top - have) if state == "Charging" else have
        if left <= 0:
            return None
        minutes = int(round(left / speed * 60.0))
        # A day is not a battery reading, it is a rate that was measured
        # during a lull. Two days would be, and either is a number nobody
        # should read off a lock screen as fact.
