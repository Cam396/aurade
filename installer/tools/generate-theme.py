...
def emit_css(palettes, light, dark) -> str:
    out = [HEADER]

    def block(scheme: dict[str, str], prefix: str) -> None:
        for name, value in scheme.items():
            out.append(f"@define-color {prefix}{name} {value};")

    out.append("\n/* ---- Material 3 roles, light ---- */")
    block(light, "m3_")
    out.append("\n/* ---- libadwaita, mapped onto the roles ---- */")
    out.append(_adw_map(light))
    out.append("\n/* ---- state layers and brand ---- */")
    out.append(f"@define-color m3_hover {rgba(light['on_surface'], STATE['hover'])};")
    out.append(f"@define-color m3_focus {rgba(light['primary'], STATE['focus'])};")
    out.append(f"@define-color m3_pressed {rgba(light['on_surface'], STATE['pressed'])};")

    out.append("\n@media (prefers-color-scheme: dark) {")
    out.append("/* GTK selects the dark variant through Adw.StyleManager; this")
    out.append("   block exists so the sheet is also correct when loaded alone. */")
    out.append("}")

    out.append("\n" + _components())
    out.append("\n/* ================= dark scheme ================= */")
    out.append(".aurade-dark {")
    out.append("}")
    return "\n".join(out) + "\n"
...
/* ---- the erase gate: the one surface that must not look inviting ------ */
...
def main() -> int:
    ...
    targets = {
        os.path.join(ROOT, "installer", "lib", "aurade_gui", "theme.css"):
            emit_css(palettes, light, dark),
        os.path.join(ROOT, "installer", "lib", "aurade_gui", "tokens.py"):
            emit_python(palettes, light, dark, brand),
    }
    stale = []
