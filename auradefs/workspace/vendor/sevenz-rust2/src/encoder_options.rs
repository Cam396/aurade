...
impl Lzma2Options {
    ...
    /// Sets the match finder's nice length, which 7-Zip calls the word size
    /// (the `fb` switch, "number of fast bytes").
    ///
    /// Will be clamped between 8..=273.
    ///
    /// AuraDE: added over 0.19.4, which exposes the dictionary size and the
    /// thread count but not this; the Create archive dialog offers all three.
    ...
    /// Sets the chunk each thread compresses on its own, for a multi threaded
    /// write. Clamped up to the dictionary size, as `from_level_mt` clamps.
    ///
    /// AuraDE: added over 0.19.4. `from_level_mt` sizes the chunk against the
    /// preset's dictionary, and a dictionary set afterwards leaves the chunk
    /// where it was; this lets the chunk follow the dictionary.
    pub fn set_chunk_size(&mut self, chunk_size: u64) {
        self.options.set_chunk_size(NonZeroU64::new(
            chunk_size.max(self.options.lzma_options.dict_size as u64),
        ));
    }

    /// The dictionary size the encoder will use, after clamping.
    pub fn dictionary_size(&self) -> u32 {
        self.options.lzma_options.dict_size
    }

    /// The nice length the encoder will use, after clamping.
    pub fn nice_len(&self) -> u32 {
        self.options.lzma_options.nice_len
    }

    /// How many threads the encoder will use.
    pub fn threads(&self) -> u32 {
        self.threads
    }

    /// The chunk each thread compresses on its own, once more than one
    /// thread is asked for; None is one stream.
    pub fn chunk_size(&self) -> Option<u64> {
        self.options.chunk_size.map(|c| c.get())
    }
}
