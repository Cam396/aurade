...
if [[ "${release_package_smoke}" == "1" ]]; then
  for package_dir in aurade-account-helper aurade-system-helper \
      ...
      aurade-login aurade-greeter aurade-ai aurade-webapp-shortcuts aurade aurade-full; do
  ...
  require_remote "AuraDE package ownership and integrity" \
    "pacman -Qk aurade-account-helper aurade-system-helper shill-nm-adapter aurade-power aurade-host-bridge chromiumos-ash aurade-login aurade-greeter aurade-ai aurade-webapp-shortcuts aurade aurade-full > /tmp/aurade-package-audit && ! grep -Eq ', [1-9][0-9]* missing files' /tmp/aurade-package-audit && pacman -Qo /usr/bin/aurade /usr/bin/aurade-power-status /usr/bin/aurade-hostctl /usr/bin/aurade-session-control /usr/lib/chromiumos-ash/chrome >/dev/null"
